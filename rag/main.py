...
async def score_cv_vs_offer(
    ...
    try:
        ...
        if result.get("status") == "ERROR":
            return {
                "status": "ERROR",
                "message": result.get("explanation", "Erreur de scoring"),
                "data": result
            }
...
def startup_checks():
    ...
    if not settings.GEMINI_API_KEY:
        logger.warning(
            ...
            "   Le service utilisera le mode local (fallback) pour le scoring.\n"
            "   Pour activer Gemini, définissez GEMINI_API_KEY dans .env"
        )
    else:
        logger.info(f"✅ GEMINI_API_KEY configurée: {settings.GEMINI_API_KEY[:15]}...")
