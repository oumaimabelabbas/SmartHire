...
class CVScoringService:
    ...
    def score_cv_vs_offer(cv: ExtractedCVDTO, offer: NormalizedOfferDTO) -> Dict[str, Any]:
        try:
            ...
            try:
                ...
                logger.warning("Scoring Gemini indisponible, fallback local actif: %s", exc)
                return CVScoringService._local_score_cv_vs_offer(cv, offer, evidence_chunks, str(exc))

        except Exception as exc:
            logger.error("Scoring LLM-RAG failed: %s", exc)
            return CVScoringService._local_score_cv_vs_offer(
                cv,
                offer,
                CVScoringService._fallback_evidence_chunks(cv, offer),
                f"Erreur lors du calcul du score: {exc}",
            )
    ...
    def answer_chatbot_question(
        ...
        try:
            ...
            logger.warning("Chatbot Gemini indisponible, fallback local actif: %s", exc)
            raw = CVScoringService._local_chatbot_cv_improvement(
                question=question,
                cv=cv,
                offer=offer,
                evidence_chunks=evidence_chunks,
    ...
    @staticmethod
    def _local_chatbot_cv_improvement(
        question: str,
        cv: ExtractedCVDTO,
        offer: NormalizedOfferDTO,
        evidence_chunks: List[str],
        ...
        required_technologies, required_skills = CVScoringService._split_offer_requirements(offer)
        strengths = CVScoringService._unique_strings(list(cv.technologies or []) + list(cv.competences or []))[:CVScoringService._CHATBOT_MAX_ITEMS]
        focus_items = (required_technologies[:3] + required_skills[:3])[:CVScoringService._CHATBOT_MAX_ITEMS]

        answer = (
            f"Mets en avant {', '.join(strengths) if strengths else 'tes realisations cles'}. "
            f"Priorite offre: {', '.join(focus_items) if focus_items else 'competences principales'}."
        )

        action_items = []
        for item in focus_items[:CVScoringService._CHATBOT_MAX_ITEMS]:
            action_items.append(f"Mettre en avant {item}")
        if not action_items:
            action_items = ["Ajouter des exemples concrets", "Adapter le CV à l'offre cible"]

        improvement_areas = []
        for item in focus_items[:CVScoringService._CHATBOT_MAX_ITEMS]:
            improvement_areas.append(f"Renforcer {item}")
        if not improvement_areas:
            improvement_areas = ["Clarifier les experiences les plus pertinentes"]

        rewritten_bullets = []
        for item in strengths[:CVScoringService._CHATBOT_MAX_ITEMS]:
            rewritten_bullets.append(f"{item}: illustrer par un résultat mesurable")

        return {
            "answer": answer,
            "strengths": strengths[:CVScoringService._CHATBOT_MAX_ITEMS],
            "improvement_areas": improvement_areas[:CVScoringService._CHATBOT_MAX_ITEMS],
            "action_items": action_items[:CVScoringService._CHATBOT_MAX_ITEMS],
            "rewritten_bullets": rewritten_bullets[:CVScoringService._CHATBOT_MAX_ITEMS],
            "retrieved_evidence": evidence_chunks,
            "history": history[-6:],
        }
