...
class LocalVectorStore:
    ...
    def close(self) -> None:
        """Release Chroma resources so Windows can clean temporary folders."""
    ...
    def upsert_chunks(
        ...
        if ids:
            try:
                ...
                if "dimension" in message and self._client is not None:
                    logger.warning("Vector collection dimension mismatch, recreating collection: %s", exc)
                    self._recreate_collection()
                    if self._collection is None:
                        self._mode = "memory"
                        self.upsert_chunks(doc_id, chunks, vectors, metadata)
                        return
                    self._collection.upsert(
                        ids=ids,
                        documents=documents,
    ...
    def _recreate_collection(self) -> None:
        ...
        try:
            self._client.delete_collection(name="cv_chunks")
        except Exception:
            pass
        ...
        try:
            ...
            logger.warning("Unable to recreate vector collection, switching to memory mode: %s", exc)
            self._collection = None
